...
## E01 轮次收口（2026-09-21 11:20，commit 751235a）

**本轮范围**：补标枚举修正 + 粗类概率聚合修正 + 候选库可改进上界诊断（只读），不重训、不扩港、不放宽词表。

### 两项实现修正
- ： 完整候选集（替换 ），T1 150m/180s、T2 500m/300s 分档独立证据；114 产品 T1 19,361 / T2-only 4,964；(product,MMSI) 复用 11,022 对（待核）。旧层保留。
- 粗类评估改为**先按父类求和再 argmax**。
...
### 结论修正（措辞，记录在案）
1. Jebel Ali **未达**「每类≥50」（fishing 4–6）；门槛不重新启用
2. B0 vs B1 差（+3.6 点）**不作**「噪声边缘」结论，且非知识增量（B0=选出的单头，B1=8 头等权）
3. 零增量已由  确定，与新标签量无关；新标签价值在检查其它候选与错误类型
...
### 磁盘
服务器 930G 中 858G 已用 / **73G 可用（93%）**，夸克归档仍在跑，需盯余量；可删候选见  第四节（等确认）。
