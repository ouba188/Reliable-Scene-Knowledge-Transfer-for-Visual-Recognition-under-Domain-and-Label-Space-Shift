...
ARMS = CANDS + ['gate3', 'gate2', 'oracle3', 'oracle2']
...
out = {a: [] for a in ARMS}
dump = []          # per-instance decisions, for the margin analysis
dump_pc = []       # per port-class rates
print('%-18s' % 'port' + ''.join('%8s' % nm for nm in ARMS))
print('-' * 62)
for p in PORT_U:
    ...
    if F:
        ...
        ok3 = np.zeros(len(te), bool); ok2 = np.zeros(len(te), bool)      # ponytail: -1 sentinel => counts as wrong
        for nm in CANDS:
            ok3 |= (pa[nm] == y[te])
        for nm in ['ridge_VK', 'gbm_zk']:
            ok2 |= (pa[nm] == y[te])
        preds['oracle3'] = np.where(ok3, y[te], -1)
        preds['oracle2'] = np.where(ok2, y[te], -1)
        dump.append({'port': p, 'y': y[te], 'pred': {nm: pa[nm] for nm in CANDS},
                     'score': scores, 'pick3': pick3, 'pick2': pick2})
    else:
        preds['gate3'] = preds['ridge_VK']; preds['gate2'] = preds['gbm_zk']
        preds['oracle3'] = preds['ridge_VK']; preds['oracle2'] = preds['ridge_VK']
    for nm in ARMS:
    ...
    for c in range(C):
        ...
        if m_.any():
            for nm in ARMS:
                dump_pc.append((p, c, int(m_.sum()), float((preds[nm][m_] == c).mean())))
    print('%-18s %7.3f %8.3f %7.3f %7.3f %7.3f %6.3f %6.3f' % (p, *[out[nm][-1] for nm in ARMS]), flush=True)

import csv as _csv, pickle
pickle.dump(dump, open('e61_preds.pkl', 'wb'))
with open('e61_perclass.csv', 'w', newline='', encoding='utf-8') as fh:
    w = _csv.writer(fh); w.writerow(['port', 'class', 'n'] + ARMS)
    keys = sorted(set((x[0], x[1], x[2]) for x in dump_pc))
    lut = {}
    for i, nm in enumerate(ARMS):
        for x in dump_pc[i::len(ARMS)]:
            lut[(x[0], x[1], nm)] = x[3]
    for (pp, cc, nn) in keys:
        w.writerow([pp, cc, nn] + ['%.5f' % lut.get((pp, cc, nm), float('nan')) for nm in ARMS])
print('per-class grid -> e61_perclass.csv (%d port-class cells)' % len(keys))
print()
for nm in ARMS:
