...
def finetune(tr, epochs, dev):
    ...
    B = 8       # small batch: Windows WDDM makes large CUDA allocations flaky here
...
def main():
    ...
    try:
        ...
        if av < 2.0:
            print('!! 需要至少 2GB 主机内存（当前 %.2f GB）。请先关闭夸克客户端/多余浏览器/WSL 再跑。' % av)
            return
    except Exception:
        pass
    torch.set_num_threads(4)          # 24 线程在 15.6GB 机器上会放大内存占用
    ...
    for p in PLIST[:nfolds]:
        ...
        res['V'].append(ba(y[te], pv)); res['K'].append(ba(y[te], pk))
        print('   fold %s: V %.4f  K %.4f  Δ %+.4f   (%.0f s)' % (
            p, res['V'][-1], res['K'][-1], res['K'][-1] - res['V'][-1], time.time() - t0), flush=True)
    print('\n=== fine-tuned baseline (%d folds) ===' % len(res['V']), flush=True)
    print('  V %.4f  K %.4f  Δ(K−V) %+.4f' % (
        float(np.mean(res['V'])), float(np.mean(res['K'])),
        float(np.mean(res['K'])) - float(np.mean(res['V']))), flush=True)
